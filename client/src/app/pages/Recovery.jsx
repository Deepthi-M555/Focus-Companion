                  ...
                  <button 
                    onClick={async () => {
                        ...
                        try {
                            ...
                            toast.error("Unable to recover your schedule.");
                        } finally {
        ...
        {step === 2 && (
          ...
          >
            <div className="text-center mb-8">
              ...
              <p className="text-neutral-500">{recoveryResult?.recovered ? "I compressed the lower-priority work and preserved your completed tasks." : "I kept your remaining tasks intact and preserved your completed work."}</p>
            </div>
            ...
            <div className="bg-white/80 dark:bg-neutral-900/80 backdrop-blur-xl border border-neutral-200 dark:border-neutral-800 rounded-3xl p-6 shadow-2xl mb-8 flex gap-6">
              <div className="flex-1 opacity-50 border-r border-neutral-200 dark:border-neutral-800 pr-6">
                ...
                <div className="space-y-3">
                  {recoveryResult?.tasks?.slice(0, 3).map((task, index) => (
                    <div key={task._id || task.id || index} className="p-3 bg-neutral-100 dark:bg-neutral-800 rounded-lg text-sm">
                      {task.title || task.name || "Untitled task"} {task.estimatedDuration ? `(${task.estimatedDuration}m)` : ""}
                    </div>
                  )) || (
                    <div className="p-3 bg-neutral-50 dark:bg-neutral-800/50 rounded-lg text-sm">No remaining tasks to recover.</div>
                  )}
                </div>
              </div>
              ...
              <div className="flex-1">
                ...
                <div className="space-y-3">
                  {recoveryResult?.suggestions?.length ? recoveryResult.suggestions.map((suggestion, index) => (
                    <div key={`${suggestion}-${index}`} className="p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 rounded-lg text-sm font-medium text-blue-700 dark:text-blue-300">
                      {suggestion}
                    </div>
                  )) : (
                    <div className="p-3 bg-neutral-50 dark:bg-neutral-800/50 border border-neutral-100 dark:border-neutral-800 rounded-lg text-sm">
                      Your schedule was preserved without extra adjustments.
                    </div>
                  )}
                </div>
              </div>
